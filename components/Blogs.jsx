...
const Blogs = ({ page }) => {
  ...
  return (
    <>
      ...
      <div className="w-full h-full p-3 xl:p-6 rounded-md flex gap-10">
        {blogs?.map((blog, index) => {
          const userData = blog?.userData;
          return (
            <Link href={`/blogs/${blog?._id}`} target="_blank">
              <div className="flex w-[350px] h-full gap-5" key={blog._id}>
                <div className="h-[450px] shadow-lg w-full rounded-md flex flex-col gap-4 p-4 cursor-pointer">
                  {/* Image */}
